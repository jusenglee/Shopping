...
// 데이터를 가져오는 함수
const getItem = () => {
  api.get('/seller/itemManage/sellerItemList').then(response => {
...
async function deleteItem(itemId) {
  try {
    await api.delete(`/seller/manage/delete/${itemId}`);
    getItem();
  } catch (error) {
...
async function stopSellingIterm(itemId) {
  try {
    await api.put(`/seller/manage/stopSellItem/${itemId}`);
    getItem();
  } catch (error) {
    console.log(error.response);
    alert(error.response.data.message)
        ...
        <tbody id="productTable">
          ...
          >
            ...
            <td>
              ...
              >
                <router-link
                  :to="`/seller/SellerModifyItemPage/${product.id}`"
                  class="button"
                  data-text="Modified"
